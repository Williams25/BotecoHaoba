	...
	private class ActionOk implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			Comanda comanda = new Comanda(Integer.parseInt(textMesa.getText().trim()),
					Integer.parseInt(textPessoas.getText().trim()), textCliente.getText());

			// testar valores inválidos
			if (comanda.getMesa() < 0 || comanda.getMesa() > 10) {
				JOptionPane.showMessageDialog(botaoOk, "Preencha a mesa corretamente! \n A mesa deve ser de 0 até 10");
			} else if (comanda.getQuantidadePessoasMesa() <= 0) {
				JOptionPane.showMessageDialog(botaoOk, "Preencha a Quantidade de pessoas corretamente!");
			} else if (facade.verificaMesa(comanda)) {
				JOptionPane.showMessageDialog(botaoOk, "Mesa já está em uso, verifique a mesa digitada.");
			} else {
